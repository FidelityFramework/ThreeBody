...
B-posit is a draft specification (Jonnalagadda, Thotli, and Gustafson, currently in review as of early 2026) that addresses the main practical objection to posit hardware. Standard posits use a variable-length regime field that requires sequential decoding, making them slower than IEEE floats in silicon. B-posit bounds that regime field to a fixed width, which turns the decode into a simple parallel MUX operation rather than a sequential one. This changes the conversation from "posits are theoretically better but impractical" to "posits are practical for the workloads that matter." (Quantitative decode-speed figures are still in review and are deliberately not cited here as fact; the demo does not load-bear on a specific percentage.)
...
**The quire accumulator.** B-posits define a fixed-width accumulator (the quire) that provides *exact* accumulation. For b-posit the quire is a fixed 800 bits (a vector of 25 32-bit integers, independent of precision). A sum of products computed in the quire loses zero precision at any intermediate step; the only rounding occurs at the final conversion back to a b-posit value, and every intermediate result is exact. This is a mathematical property of the format, not an engineering aspiration — and it is the single property the demo load-bears on. It is exactness of *accumulation*, full stop: it is what survives the close-encounter subtraction `qᵢ−qⱼ` where IEEE FP64 catastrophically cancels. (Note: this is not a claim about precision near zero. Posits taper toward magnitude 1.0 and *lose* precision near zero like every representation; the quire's value is exact accumulation, not near-zero precision.)
